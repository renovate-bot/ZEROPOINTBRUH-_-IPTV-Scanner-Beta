...
async def run_epg_refresh(store: Optional[ChannelStore] = None) -> dict:
    """Placeholder EPG refresh.

    We don't yet ship an EPG pipeline; this stub exists so scheduling code
    can be wired end-to-end today and a future patch can drop in the real
    XMLTV/JSON refresh without touching the task registry.
    """
    log.info("epg-refresh: no-op (EPG pipeline not implemented yet)")
    task_queue.log_worker_batch(
        "epg-refresh",
        ["EPG pipeline not implemented yet — noop"],
        limit=1,
    )
    return {"status": "noop"}
